...
export async function PublicFooter() {
  ...
  const orgName = settings?.orgName ?? "Liga AEFPY";
  const orgTagline = settings?.orgTagline ?? "Asociación de Efootball Paraguay";
  ...
  return (
    <footer className="mt-auto border-t border-white/10 bg-[var(--color-navy-950)]">
      <div className="container-page flex flex-col gap-6 py-8 md:flex-row md:items-start md:justify-between">
        <div>
          <Logo size={34} variant="light" name={orgName} tagline={orgTagline} />
          <p className="mt-3 max-w-xs text-[0.8rem] leading-relaxed text-white/60">{footerDescription}</p>
        </div>
